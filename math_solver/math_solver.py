...
class State(rx.State):
    ...
    def solve_trig(self):
        ...
        if self.trig_func == "sine":
            ...
            if -1 <= value <= 1:
                result_deg = math.degrees(
                    math.asin(value)
                )
                self.trig_result = str(
                    round(result_deg, 4)
                )
            else:
                self.trig_error = (
    ...
    def solve_quadratic(self):
        ...
        if a == 0:
            if b != 0:
                ...
                self.quad_info = (
                    "a = 0, so this is actually linear."
                )
            else:
                self.quad_error = (
    ...
    def solve_stats(self):
        ...
        try:
            data_list = [
                float(v.strip())
                for v in self.stats_raw.split(",")
                if v.strip()
            ]
        except ValueError:
            self.stats_error = (
                "Please ensure you only use "
                "numbers and commas"
            )
            return
        ...
        self.stats_mean = (
            f"{np.mean(data_list):.2f}"
        )

        self.stats_median = (
            f"{np.median(data_list):.2f}"
        )

        self.stats_std = (
            f"{np.std(data_list):.2f}"
        )
    ...
    def convert_length(self):
        ...
        try:
            ...
            result = (
                value
                * LENGTH_UNITS[self.len_from]
                / LENGTH_UNITS[self.len_to]
            )

            self.unit_result = (
                f"{value} {self.len_from} = "
                f"{result:.4f} {self.len_to}"
            )
    ...
    def convert_weight(self):
        ...
        try:
            ...
            result = (
                value
                * WEIGHT_UNITS[self.wt_from]
                / WEIGHT_UNITS[self.wt_to]
            )

            self.unit_result = (
                f"{value} {self.wt_from} = "
                f"{result:.4f} {self.wt_to}"
            )
    ...
    def convert_temp(self):
        ...
        # Convert Celsius to destination temperature.
        ...
        self.unit_result = (
            f"{value} {self.temp_from} = "
            f"{result:.2f} {self.temp_to}"
        )
    ...
    def convert_data(self):
        ...
        try:
            ...
            result = (
                value
                * DATA_UNITS[self.data_from]
                / DATA_UNITS[self.data_to]
            )

            self.unit_result = (
                f"{value} {self.data_from} = "
                f"{result:.2f} {self.data_to}"
            )
...
def simultaneous() -> rx.Component:
    return rx.vstack(
        ...
        rx.button(
            "Solve",
            on_click=State.solve_simultaneous,
            color_scheme="blue",
        ),
...
def quadratics() -> rx.Component:
    return rx.vstack(
        ...
        rx.button(
            "Solve",
            on_click=State.solve_quadratic,
            color_scheme="blue",
        ),
...
def calculus() -> rx.Component:
    return rx.vstack(
        ...
        labeled_input(
            "Enter function",
            State.calc_input,
            State.set_calc_input,
        ),

        rx.button(
            "Solve",
            on_click=State.solve_calculus,
            color_scheme="blue",
        ),
